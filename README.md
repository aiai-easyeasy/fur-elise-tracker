...
- **只顯示今天還沒來的班次**：已過的時段與今天不收的點會隱藏起來，按「顯示全部」看完整時刻表（選擇會記住）。清單固定就是最近 20 個點、隱藏後不會從更遠的地方補位，所以「已隱藏 N 個」剛好等於清單少掉的列數
- **時間自己更新**：每到整分就重算「還有幾分鐘」，從背景切回來也會立刻重算，不用重開頁面
- 右上角搜尋街名、巷弄或收運點
- 點選收運點：時段、路線／車次／車號，以及完整站序（本站、上一站、下一站）；點進去的頁面一律列出完整時段，包含已過的
- 「路線」按鈕在地圖上畫出這台車的行駛順序
- 週三、週日開啟時顯示「今日不收運」
- 附近的班次都跑完時清單不會變成空白，改為顯示完整時刻表並註明那是明天的班次
- 網址後面加 `?day=3`（星期三）或 `?now=23:50` 可以模擬指定的星期與時間，用來確認上面這些狀態
...
頁面本身還會**把同一個點的多筆資料合併**：原始檔常讓不同車次各自標一組座標，同一個地址因此相差幾公尺到幾十公尺（例如大安區通化街167號前，19:05 那班標 121.55387,25.02833、22:24 那班標 121.55385,25.0283，相距 3.9 公尺）。名稱相同且相距 120 公尺內的視為同一個收運點，時段合併在一起、位置取平均，共 49 個點。相距更遠的同名地址則是座標有誤而不是重複，維持分開。
...
- 有 3 個地址的兩筆座標相差數百公尺以上（中正區忠孝東路2段66號差 1.8 公里、北投區大業路大興街口差 260 公尺、文山區忠順街1段26巷11弄2號前差 224 公尺）。無法判斷哪一筆才對，合併只會把點放到兩者中間，因此頁面上仍是兩個點。
